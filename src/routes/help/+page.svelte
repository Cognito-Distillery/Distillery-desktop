<script lang="ts">
	...
	const glossary: { titleKey: MessageKey; descKey: MessageKey }[] = [
		{ titleKey: 'help.g.malting', descKey: 'help.g.malting.d' },
		{ titleKey: 'help.g.malt', descKey: 'help.g.malt.d' },
		{ titleKey: 'help.g.maltHouse', descKey: 'help.g.maltHouse.d' },
		...
		{ titleKey: 'help.g.queue', descKey: 'help.g.queue.d' },
		{ titleKey: 'help.g.distill', descKey: 'help.g.distill.d' },
		{ titleKey: 'help.g.cask', descKey: 'help.g.cask.d' },
		{ titleKey: 'help.g.drawBack', descKey: 'help.g.drawBack.d' }
	];

	const maltTypes: { badgeClass: string; typeKey: MessageKey; descKey: MessageKey }[] = [
...
<div class="max-w-2xl mx-auto flex flex-col gap-8">
	...
	<section class="flex flex-col gap-6">
		...
		<div class="border border-base-300 rounded-lg p-5 flex flex-col gap-2">
			<a href="/malts" class="text-sm font-semibold flex items-center gap-2 text-base-content hover:!text-primary transition-colors">
		...
		<div class="border border-base-300 rounded-lg p-5 flex flex-col gap-2">
			<a href="/drawback" class="text-sm font-semibold flex items-center gap-2 text-base-content hover:!text-primary transition-colors">
				<span class="w-6 h-6 flex items-center justify-center rounded bg-primary/15 text-primary text-xs font-bold">4</span>
				{t('help.step4.title')}
			</a>
			<p class="text-sm text-base-content/60 leading-relaxed">
				{@html t('help.step4.desc1')}
			</p>
			<p class="text-sm text-base-content/60 leading-relaxed">
				{@html t('help.step4.desc2')}
			</p>
		</div>
	</section>
	...
	<section class="border border-base-300 rounded-lg p-5 flex flex-col gap-3">
		<h2 class="text-xs font-medium text-base-content/40 uppercase tracking-wider">{t('help.maltTypes')}</h2>
